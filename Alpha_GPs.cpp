          ...
          if (!fixedNoise)
            {
              ...
              trace -= g(index-1);
              //for ( auto j : boost::irange(0, n) )
              // trace += 0.5*params(0)*std::pow(_alpha(j),2);
              g(index++) = trace;
            }
          else
            {
              ...
              for ( auto j : boost::irange(0, n) )
                {
                  ...
                  Matrix tmp = _alpha.transpose()*dK.col(j);
                  trace += _alpha(j)*tmp(j);
                }
              g(index++) = 0.5*trace;
            }
          //  
      ...
      for (auto dK_i = gradList.begin(); dK_i != gradList.end(); ++dK_i) 
        {
          ...
          Matrix alpha_dK = _alpha.transpose()*(*dK_i);
            
          // Define lambda function specifying each threads solver task
          auto lambda = [&_alpha,&alpha_dK,&term,&dK_i,&traceVals](int i, int startInd, int endInd) {
                          for ( auto j : boost::irange(startInd, endInd) )
                            {
                              traceVals(i) += term.row(j)*(*dK_i).col(j);
                              traceVals(i) -= _alpha(j)*alpha_dK(j);
                            }
                        };
