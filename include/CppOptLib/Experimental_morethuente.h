...
class MoreThuente {
  ...
  */
  ...
  //static Scalar linesearch(const TVector &x, const TVector &searchDir, ProblemType &objFunc, const  Scalar alpha_init, Scalar fval, TVector gval, int &info) {
  static Scalar linesearch(const TVector &x, const TVector &searchDir, ProblemType &objFunc, const  Scalar alpha_init, Scalar &fval, TVector & gval, int &info) {
    // assume step width
    Scalar ak = alpha_init;
    ...
    //cvsrch(objFunc, xx, fval, g, ak, s);
    info = cvsrch(objFunc, xx, fval, gval, ak, s);
  ...
  //static int cvsrch(ProblemType &objFunc, TVector &x, Scalar f, TVector &g, Scalar &stp, TVector &s, Criteria<Scalar> &stop, Criteria<Scalar> &current, Status &status) {    
    ...
    // MODIFIED
    const Scalar xtol   = 5e-1;
    const Scalar ftol   = 5e-1;
    const Scalar gtol   = 5e-2;
    const Scalar stpmin = 1e-1;
    ...
    while (true) {
      ...
      // NOTE: I believe the initial evaluation may simply be re-evaluating the previous linesearch values
      if ( nfev == 0 )
        nfev++;
      else
        {
      ...
      if (info != 0)
        {
          ...
          //std::cout << "\n[*] Line Search Terminated\n";
          //std::cout << "Function Evaluations: " << nfev << "\t INFO = " << info << "\t x = " << x.transpose() << std::endl;
          std::cout << "\nEnd of line search: \t  [ INFO = " << info << " ] \t\t\t\t\t  [ Function Evaluations = " << nfev << " ]\n";
          double lhs;
          double rhs;
          if (info == 1)
            {
              std::cout << "\nstp = " << stp <<  " \t\t [ stpmin =  " << stpmin << " ]\n";
              lhs = f;
              rhs = ftest1;
              std::cout << lhs << " < " << rhs << "\t [ ftol = "  << ftol << " \t finit = "  << finit << " ]\n";
              lhs = fabs(dg);
              rhs = gtol * (-dginit);
            ...
            {
              //std::cout << "stpmin =  " << stpmin << "   stp = " << stp << "    stpmax = " << stpmax << std::endl;
              std::cout << "\nstp = " << stp <<  " \t\t [ stpmin =  " << stpmin << " ]\n";
              std::cout << "brackt = " << brackt <<  " \t infoc = " << infoc << std::endl;
            }
          else if (info == 4)
            {
              std::cout << "\nstp = " << stp <<  " \t\t\t\t [ stpmin =  " << stpmin << " ]\n";
              if (f > ftest1)
                std::cout << "f =  " << f << "  >  ftest1 = " << ftest1 << "\t [ ftol = "  << ftol << " \t finit = "  << finit << " ]\n";
              else if (dg >= dgtest)
                std::cout << "dg =  " << dg << "  >  dgtest = " << dgtest << "\t [ gtol = "  << gtol << " \t dginit = "  << dginit << " ]\n";
            }
          */
          return info;
          //return -1;
