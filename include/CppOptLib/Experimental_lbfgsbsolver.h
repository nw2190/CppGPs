 ...
 public:
  ...
  void minimize(TProblem &problem, TVector &x0) {
    ...
    SubspaceMinimization(problem, CauchyPoint, x, c, g, SubspaceMin);
      ...
      //TVector x_linesearch = x;
      //Scalar f_linesearch = f;
      //TVector g_linesearch = g;
      
      x = x - rate*(x-SubspaceMin);

      /*
      if ( (x_linesearch-x).squaredNorm() != 0.0 )
        {
          std::cout << "\n[*] WARNING: Unexpected line-search exit status; re-evaluating... [ INFO = " << info << " ]\n";
          f = problem.value(x);
          problem.gradient(x, g);
          std::cout << "||ls_x - updated_x|| = " << (x_linesearch-x).squaredNorm() << std::endl;
          std::cout << "||ls_f - updated_f|| = " << std::abs(f_linesearch-f) << std::endl;
          std::cout << "||ls_g - updated_g|| = " << (g_linesearch-g).squaredNorm() << std::endl;
        }
      */


      //if ( ( ( info != 1 ) && ( info != 6 ) ) && ( info != 4 ) )
      //{
      //    std::cout << "\n[*] WARNING: Unexpected line-search exit status; re-evaluating... [ INFO = " << info << " ]\n";
      //    
      //    f = problem.value(x);
